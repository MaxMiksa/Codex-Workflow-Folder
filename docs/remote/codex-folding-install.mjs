...
function patchWebviewBundleJs(source) {
  if (source.includes("CODEX_WORKFLOW_FOLD_PATCH_V9")) return source;
...
const __codexOrigIsAgentItemStillRunning=typeof isAgentItemStillRunning==="function"?isAgentItemStillRunning:null;
	...
	function __codexWorkflowItemIsRunning(it){const kids=it?.children??[];for(const ch of kids){if(!ch)continue;try{if(__codexOrigIsAgentItemStillRunning&&__codexOrigIsAgentItemStillRunning(ch))return!0}catch{}if(ch.completed===!1||ch.isPending===!0)return!0;const st=ch.status??ch.state;if(st==="inProgress"||st==="pending"||st==="running")return!0}return!1}
		function __CodexWorkflowItem(rt){const {item:it}=rt,mode=__codexWorkflowCollapseMode();if(mode==="disable")return null;const intl=useIntl();const startedAt=reactExports.useRef(Date.now()),doneAt=reactExports.useRef(null);const rootRef=reactExports.useRef(null);const [radius,setRadius]=reactExports.useState(null);reactExports.useEffect(()=>{try{const el=rootRef.current;if(!el)return;const prev=el.previousElementSibling;if(!prev)return;const r=__codexWorkflowPickBorderRadiusFromNode(prev);if(r)setRadius(r)}catch{}},[]);const isRunning=typeof it?.__codexIsRunning==="boolean"?it.__codexIsRunning:__codexWorkflowItemIsRunning(it);reactExports.useEffect(()=>{if(!isRunning&&doneAt.current==null)doneAt.current=Date.now();if(isRunning)doneAt.current=null},[isRunning]);const [now,setNow]=reactExports.useState(Date.now());reactExports.useEffect(()=>{if(!isRunning)return;const id=setInterval(()=>setNow(Date.now()),1e3);return()=>clearInterval(id)},[isRunning]);const [collapsed,setCollapsed]=reactExports.useState(it.defaultCollapsed===!0);const endTs=isRunning?now:(doneAt.current??now);const elapsed=__codexWorkflowFormatElapsedCompact(endTs-startedAt.current,intl?.locale);const icon=isRunning?"⚡":"✅";const header=jsxRuntimeExports.jsx("div",{className:"flex items-center gap-1.5",children:jsxRuntimeExports.jsx("span",{className:"truncate",children:intl.formatMessage({id:"codex.workflow.label",defaultMessage:"Workflow",description:"Label for the Workflow folding header"})})});const meta=jsxRuntimeExports.jsx("span",{className:"text-token-description-foreground whitespace-nowrap opacity-70",children:icon+" "+elapsed});const title=jsxRuntimeExports.jsxs("div",{className:"flex min-w-0 items-baseline gap-2",children:[header,meta]});const chevron=jsxRuntimeExports.jsx(typeof SvgChevronRight!=="undefined"?SvgChevronRight:"span",{className:clsx("icon-xs transition-transform",collapsed?"rotate-0":"rotate-90")});const row=jsxRuntimeExports.jsxs("div",{className:"flex w-full items-center justify-between gap-2",children:[title,chevron]});const expanded=!collapsed&&(it.children??[]).length>0?jsxRuntimeExports.jsx("div",{className:"border-token-border/80 border-t flex flex-col gap-2 px-3 py-2",style:{backgroundColor:"#212121"},children:(it.children??[]).map((ch,idx)=>jsxRuntimeExports.jsx(__codexOrigLocalConversationItemContent,{...rt,item:ch},ch?.callId??ch?.id??\`\${it.id}-\${idx}\`))}):null;return jsxRuntimeExports.jsxs("div",{ref:rootRef,className:"border-token-border/80 border overflow-hidden",style:{borderRadius:radius||"16px",marginTop:"4px",marginBottom:"12px"},children:[jsxRuntimeExports.jsx("button",{type:"button",className:"w-full px-3 py-2 text-left",style:{backgroundColor:"#212121"},onClick:()=>setCollapsed(v=>!v),children:row}),expanded]})}
...
async function main() {
  ...
  results.push({
    ...
    verifyIncludes: "CODEX_WORKFLOW_FOLD_PATCH_V9",
  });
  results.push({
    file: zhCnJs,
