...
function __CodexWorkflowItem(rt){const {item:it}=rt,mode=__codexWorkflowCollapseMode();if(mode==="disable")return null;const intl=useIntl();const startedAt=reactExports.useRef(Date.now()),doneAt=reactExports.useRef(null);const isRunning=typeof it?.__codexIsRunning==="boolean"?it.__codexIsRunning:__codexWorkflowItemIsRunning(it);reactExports.useEffect(()=>{if(!isRunning&&doneAt.current==null)doneAt.current=Date.now();if(isRunning)doneAt.current=null},[isRunning]);const [now,setNow]=reactExports.useState(Date.now());reactExports.useEffect(()=>{if(!isRunning)return;const id=setInterval(()=>setNow(Date.now()),1e3);return()=>clearInterval(id)},[isRunning]);const [collapsed,setCollapsed]=reactExports.useState(it.defaultCollapsed===!0);const statusLabel=isRunning?intl.formatMessage({id:"codex.workflow.status.working",defaultMessage:"Working",description:"Status shown in Workflow header when the agent is still running"}):intl.formatMessage({id:"codex.workflow.status.done",defaultMessage:"Done",description:"Status shown in Workflow header when the agent has finished"});const endTs=isRunning?now:(doneAt.current??now);const elapsed=__codexWorkflowFormatElapsed(endTs-startedAt.current);const header=jsxRuntimeExports.jsx("div",{className:"flex items-center gap-1.5",children:jsxRuntimeExports.jsx("span",{className:"truncate",children:intl.formatMessage({id:"codex.workflow.label",defaultMessage:"Workflow",description:"Label for the Workflow folding header"})})});const meta=jsxRuntimeExports.jsx("span",{className:"text-token-description-foreground text-xs whitespace-nowrap",children:intl.formatMessage({id:"codex.workflow.headerSuffix",defaultMessage:"({status}, {elapsed})",description:"Suffix in Workflow header with status and elapsed time"},{status:statusLabel,elapsed})});const title=jsxRuntimeExports.jsxs("div",{className:"flex min-w-0 items-baseline gap-2",children:[header,meta]});const chevron=jsxRuntimeExports.jsx(typeof SvgChevronRight!=="undefined"?SvgChevronRight:"span",{className:clsx("icon-xs transition-transform",collapsed?"rotate-0":"rotate-90")});const row=jsxRuntimeExports.jsxs("div",{className:"flex w-full items-center justify-between gap-2",children:[title,chevron]});const expanded=!collapsed&&(it.children??[]).length>0?jsxRuntimeExports.jsx("div",{className:"border-token-border/80 border-t flex flex-col gap-2 px-3 py-2",style:{backgroundColor:"#212121"},children:(it.children??[]).map((ch,idx)=>jsxRuntimeExports.jsx(__codexOrigLocalConversationItemContent,{...rt,item:ch},ch?.callId??ch?.id??\`\${it.id}-\${idx}\`))}):null;return jsxRuntimeExports.jsxs("div",{className:"border-token-border/80 border bg-token-input-background/70 rounded overflow-hidden",children:[jsxRuntimeExports.jsx("button",{type:"button",className:"w-full px-3 py-2 text-left",onClick:()=>setCollapsed(v=>!v),children:row}),expanded]})}
